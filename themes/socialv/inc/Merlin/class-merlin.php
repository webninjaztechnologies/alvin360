...
<?php
	...
	public function socialv_download_file()
	{
		if (isset($_GET["download_item"]) && !empty($_GET['download_item']) && is_user_logged_in()) {
			$this->socialv_send_download_file($_GET['download_item']);
		}
	}
	public	function socialv_send_download_file($remoteURL)
	{

		header("Content-type: application/x-file-to-save");
		header("Content-Disposition: attachment; filename=" . basename("https://assets.iqonic.design/wp/plugins/socialv/" . $remoteURL . ".zip"));
		while (ob_get_level()) {
			ob_end_clean();
		}
		readfile($remoteURL);
