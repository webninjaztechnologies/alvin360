...
class Iqonic_Extension_Minify
{
    ...
    /**
     * Minify a JavaScript source code.
     *
     ...
     *
     * // iqonicMinJs.specialVarRx = your_special_var_regexp_here
     *
     ...
     *
     * Example:
     * $parser = new Iqonic_Extension_Minify;
     * $minify_js = $parser->minify_js($fat_js);
     */
    public function minify_js($code, $singleLine = true, $keepImportantComments = true, $specialVarRx = false)
    {
        $code = trim($code);
        if ('' === $code) {
            return '';
        ...
        if (preg_match("#//[ \t]*iqonicMinJs\.specialVarRx[ \t]*=[ \t]*([\"']?)(.*)\\1#i", $code, $key)) {
            if (!$key[1]) {
                $key[2] = trim($key[2]);
    ...
    public static function iqonic_minify_css($get_css)
    {
        $iqonic_css = file_get_contents($get_css);

        $iqonic_css = self::minify_css($iqonic_css);
        
        return $iqonic_css;
    }

    public function iqonic_minfy_js($get_js)
    {


        $iqonic_js = file_get_contents($get_js);

        $minifiedJs = $this->minify_js(
            $iqonic_js,
            true,   // $singleLine
