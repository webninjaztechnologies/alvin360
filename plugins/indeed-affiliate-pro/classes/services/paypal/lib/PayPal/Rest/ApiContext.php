...
class ApiContext
{
    ...
    private function generateRequestId()
    {
        ...
        if ($pid == -1) {
            // modified since version 8.1. before the update it was only getmypid()., but this function its not available for some servers
            if ( function_exists( 'getmypid' ) ){
                $pid = getmypid();
            } else {
                $pid = uniqid();
            }
            // end of modified since version 8.1.
        }
