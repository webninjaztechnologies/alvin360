...
class Uap_Ulp extends Referral_Main
{
    ...
    public function __construct()
    {
        ...
        if ( \Indeed\Uap\Integrations::isUlpActive() === 0 ){
            return;
        }
    ...
    public function create_referral($orderId=0, $uid=0, $courseId=0)
    {
        ...
        $description = esc_html__('Course Sale', 'uap');
  			if(isset($data['amount']) && !empty($data['amount'])){
          $current_currency = self::$currency;
  				$description = esc_html__('Order Amount: ', 'uap').'<strong>'.$data['amount'].$current_currency.'</strong>';
  			}
        // insert referral
        $args = array(
