...
/* interactive wysiwyg 4.6*/
	BB.on('click','.evo_elm_act_on',function(){
		$(this).parent().siblings('.evo_field_container').show();
		$(this).hide();
	});
...
// tooltips
	...
	.on('mouseout','.ajdeToolTip, .colorselector',function(){	
		$('.evo_tooltip_box').removeClass('show');
	});

	// appearance color circle label
	$('body').on('mouseover','.colorselector',function(event){
		event.stopPropagation();
		if($(this).hasClass('show')) return;
