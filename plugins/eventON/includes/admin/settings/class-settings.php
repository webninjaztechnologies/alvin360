		...
		if( isset($_POST['evcal_noncename']) && isset( $_POST ) ){
			if ( wp_verify_nonce( $_POST['evcal_noncename'], AJDE_EVCAL_BASENAME ) ){
				...
				// fields to skip sanitization @u 4.6
					$none_san_fields = apply_filters('evo_settings_non_san_fields', array('evo_etl','evcal_top_fields','evcal_sort_options'), $focus_tab);
				...
				foreach($_POST as $pf=>$pv){
					// skip fields
					if(in_array($pf, array('evo_ecl','option_page', 'action','_wpnonce','_wp_http_referer','evcal_noncename',
					))) continue;
					...
					// HTML fields 4.6
					if( in_array( $pf, $html_fields )){
						$evcal_options[$pf] = $help->sanitize_html( $_POST[ $pf ] );
					}			
				}
				...
				elseif($focus_tab == 'evcal_1' || empty($focus_tab)){
					...
					$__evo_ecl = EVO()->cal->get_prop( 'evo_ecl' );
					$evcal_options['evo_ecl'] = $__evo_ecl;


					EVO()->cal->set_option_values( $evcal_options );
