<?php

use IR\Admin\Classes\IR_Database;

defined('ABSPATH') || exit;
function comment_reaction_list($activity_id, $user_id, $comment_id, $is_reacted = false)
{
    $db_obj = new IR_Database();
    $reaction_list = $db_obj->getCommentsReactionList($activity_id, $comment_id);
    $total_count = count($reaction_list);
    $count = 1;
    if ($reaction_list) {
?>
        <div class="ir-comment-box">
            <div class="iqonic-meta-details comment-reaction-details">
                <div class="emoji-reaction" data-activity_id="<?php echo esc_attr($activity_id); ?>" data-reaction_id="<?php echo esc_attr($user_id); ?>" data-comment_id="<?php echo esc_attr($comment_id); ?>">
                    <div class="liked-member">
                        <ul class="member-thumb-group list-img-group">
                            <?php
                            foreach ($reaction_list as $key => $value) {
                                if (!isset($value->image_url) ||  empty($value->image_url)) continue;
                                if (isset($reaction_list[$key + 1]->id)) {
                                    if ($reaction_list[$key + 1]->id == $value->id) {
                                        continue;
                                    } else {
                                        if (isset($reaction_list[$key + 2]->id)) {
                                            if ($reaction_list[$key + 2]->id == $value->id) {
                                                continue;
                                            }
                                        }
                                    }
                                } ?>
                                <li>
                                    <img src="<?php echo esc_url($value->image_url); ?>" alt="<?php echo esc_attr($value->name); ?>">
                                </li>
                            <?php
                                if ($count >= 3) break;
                                else $count++;
                            } ?>
                        </ul>
                    </div>
                </div>

                <span class="total-member">
                    <?php if ($total_count >= 1) {
                        echo esc_html__("Reacted by ", IQONIC_REACTION_TEXT_DOMAIN);
                        foreach ($reaction_list as $value) {
                            $last_user_id = $is_reacted ? $user_id : $value->user_id;
                    ?>
                            <a href="<?php echo esc_url(bp_members_get_user_url($last_user_id)); ?>">
                                <?php echo bp_core_get_user_displayname($last_user_id); ?>
                            </a> <?php
                                    break;
                                }
                            }

                            if ($total_count > 1) {
                                $total_count = $total_count - 1;
                                esc_html_e("And ", IQONIC_REACTION_TEXT_DOMAIN);
                                echo '<span class="other-content"' . 'data-activity_id="' . esc_attr($activity_id) . '" ' . 'data-comment_id="' . esc_attr($comment_id) . '">';
                                echo esc_html($total_count);
                                if ($total_count == 1)
                                    esc_html_e(" Other", IQONIC_REACTION_TEXT_DOMAIN);
                                else
                                    esc_html_e(" Other's ", IQONIC_REACTION_TEXT_DOMAIN);
                                echo '</span';
                            } ?>
                </span>
            </div>
        </div> <?php
            }
        }

        add_action("iqonic-comment-reaction-list", "comment_reaction_list", 10, 4);
